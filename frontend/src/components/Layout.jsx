...
const Layout = ({user , onLogout  }) => {
    ...
    const stats = useMemo(()=>{
        ...
        const completedTasks = tasks.filter((task)=>{
            task.completed == true ||
            task.completed == 1 || 
            (typeof task.completed === 'string' && task.completed.toLowerCase() === 'yes').length
        ...
        return(
            <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
                 <div className='flex items-center flex-col px-8 md:px-32 bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md '>
                   ...
                   <button onClick={fetchTasks} className='mt-4 py-2 px-4 bg-red-100 text-red-600 rounded-lg text-sm font-medium hover:bg-red-200 transition-colors'>Try Again</button>
                 </div>
            </div>
        )
    }
                      ...
                      <div className='grid grid-cols-2 gap-4 sm:gap-4 mb-4 sm:mb-6'>
                            <StatCard title='total Tasks' value={stats.totalCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 text-purple-500' />} />
                             <StatCard title='Completed' value={stats.completedTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 text-green-500' />} />
                              <StatCard title='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 text-purple-500' />} />
                     ...
                     <div className='space-y-2 sm:space-y-3'>
                        ...
                        {tasks.length === 0 && (
                            <div className='text-center py-4 sm:py-6 px-2'>
                                 <div className='w-12 h-12 sm:w-16 sm:h-16 mx-auto sm:mb-4 rounded-full bg-purple-200 flex items-center justify-center'>
